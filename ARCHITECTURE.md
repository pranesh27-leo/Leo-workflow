...
Portability floor: **node 14** and git. No dependencies, and none are coming:
leo is a tool for keeping a repository honest, and one with a supply chain of
its own would be funny in the wrong way.
...
| library | `src/lib/*.js` | every shared fact: repo, plans, tasks, records, reviews, session, tools | run anything the user asked for |
| commands | `src/cmd/<name>.js` | one verb each | know about each other, except by requiring one deliberately |
| adapters | `src/integrations/<name>.js` | naming one external tool: detect, hint, install, advise | install, launch, wrap or configure anything |
| templates | `templates/` | every file leo writes into a repository | be read directly — go through `tmpl_cat` |
| asset seam | `src/cli.js`, one block | the only place leo reads its own install | be bypassed, ever |

**There is no registry.** A command is a file in `src/cmd/`; a capability is
a file in `src/integrations/`; a rule is a file in `.leo/rules/`. Adding one
...
2. `src/lib/ctx.js` builds the context. It computes `root`, resolves the **active plan**,
   ...
   EXIT trap.
3. `src/cmd/<name>.js` is required and its `run(ctx)` is called.
...
**The plan registry.** `plan` and `tasks` are resolved once, in `src/lib/ctx.js`,
