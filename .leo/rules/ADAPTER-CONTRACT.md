...
`<name>_hint`, and `<name>` appears in `CAPS` in `core/lib.sh`.

`core/lib.sh` sources that directory blind — same as `core/cmd/`, no registry —
so an adapter that is misnamed, or names a capability `CAPS` never lists, loads
...
```sh
caps=$(sed -n 's/^CAPS="\(.*\)"/\1/p' core/lib.sh)
bad=""
